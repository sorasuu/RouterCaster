    ...
    @objc func handleSignInWithFacebookButtonTapped() {
        ...
        Spark.signInWithFacebook(in: self) { (message, err, sparkUser) in
            ...
            guard let sparkUser = sparkUser else {
                SparkService.dismissHud(self.hud, text: "Error", detailText: "Failed to fetch user", delay: 0.5)
                return
            }
            
            print("Successfully signed in with Facebook with Spark User: \(sparkUser)")
            SparkService.dismissHud(self.hud, text: "Success", detailText: "Successfully signed in with Facebook", delay: 0.5)
...
//                        self.presentingViewController?.dismiss(animated: false, completion: {
//                            let appDelegate = UIApplication.shared.delegate as! AppDelegate
//                            appDelegate.window!.rootViewController!.present(messageVC, animated: false, completion: nil)
//                        })
//                    })
//                }
//            }
...
//            })
    ...
    @objc func dismissKeyboard (_ sender: UITapGestureRecognizer) {
        ...
        self.emaillogTextField.resignFirstResponder()
        self.passwordTextField.resignFirstResponder()
        self.passwordlogTextField.resignFirstResponder()
    }
    ...
    @objc func handleLogin() {
        guard let email = emaillogTextField.text else { return }
        guard let password = passwordlogTextField.text else { return }
    ...
    func logUserIn(withEmail email: String, password: String) {
        hud.textLabel.text = "Signing in with Email..."
        hud.show(in: view)
...
extension LoginViewController: GIDSignInDelegate {
    func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!, withError error: Error!) {
        ...
        Auth.auth().signInAndRetrieveData(with: credential) { (result, error) in
            ...
            let values = ["email": email, "username": username,"uid": uid, accountType:"gg"]
